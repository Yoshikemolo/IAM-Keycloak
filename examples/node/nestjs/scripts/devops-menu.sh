#!/usr/bin/env bash
#
# devops-menu.sh -- Interactive DevOps Menu for NestJS IAM Resource Server
#
# This script provides a numbered menu of common development and operations
# tasks for the NestJS IAM example project. It wraps npm, Docker, and
...
#
# The script must be run from the project root directory (examples/node/nestjs).
#
...
PROJECT_NAME="NestJS IAM Resource Server"

# Docker image name for the application
DOCKER_IMAGE="iam-nestjs"
...
# Option 3: Run the NestJS application in development mode
run_application() {
    info "Starting the NestJS application in development mode..."
    cd "$PROJECT_ROOT"
    npm run start:dev
}
...
# Option 7: Generate Compodoc documentation
generate_docs() {
    info "Generating Compodoc documentation..."
    cd "$PROJECT_ROOT"
    npm run doc
    success "Documentation generated at documentation/index.html"
}
...
# Option 10: Lint the source code
lint_code() {
    info "Running ESLint..."
    cd "$PROJECT_ROOT"
    npm run lint
...
# Option 13: Clean build artifacts and dependencies
clean_build() {
...
main() {
    ...
    while true; do
        ...
        echo "   7) Generate documentation (Compodoc)"
        echo "   8) Build Docker image (docker build)"
        echo "   9) Run with Docker Compose (docker-compose up)"
        echo "  10) Lint code (npm run lint)"
        echo "  11) View application logs (docker logs)"
        echo "  12) Stop all containers (docker-compose down)"
        echo "  13) Clean (rm -rf dist node_modules)"
        echo "   0) Exit"
        echo ""
