#!/usr/bin/env bash
###############################################################################
# devops-menu.sh -- DevOps helper menu for the Next.js 15 IAM-Keycloak example
###############################################################################
set -euo pipefail
...
print_header() {
    ...
    echo -e "${BLUE}${BOLD}"
    echo "============================================================"
    echo "  Next.js 15 -- IAM-Keycloak DevOps Menu"
    echo "============================================================"
    echo -e "${NC}"
...
run_e2e_tests() {
    cd "${PROJECT_DIR}"
    run_cmd "E2E tests (Playwright)" npm run test:e2e
}

generate_coverage() {
    cd "${PROJECT_DIR}"
    run_cmd "Coverage report generation" npm run test:cov
}
...
start_production() {
    cd "${PROJECT_DIR}"
    print_info "Starting Next.js production server..."
    print_info "Press Ctrl+C to stop."
    echo ""
    npm start
}
...
build_docker_image() {
    cd "${PROJECT_DIR}"
    run_cmd "Docker image build" docker build -t nextjs-iam .
}
...
analyze_bundle() {
    cd "${PROJECT_DIR}"
    run_cmd "Bundle analysis" npm run analyze
}
...
clean_project() {
    cd "${PROJECT_DIR}"
    print_warn "This will remove .next/ and node_modules/ directories."
    read -rp "Are you sure? [y/N]: " confirm
    if [[ "${confirm}" =~ ^[Yy]$ ]]; then
        rm -rf .next node_modules
        print_success "Cleaned .next/ and node_modules/."
    else
...
main() {
    ...
    while true; do
        ...
        echo -e "  ${BOLD} 4)${NC} Run unit tests"
        echo -e "  ${BOLD} 5)${NC} Run E2E tests (Playwright)"
        echo -e "  ${BOLD} 6)${NC} Generate coverage report"
        echo -e "  ${BOLD} 7)${NC} Build production bundle (npm run build)"
        echo -e "  ${BOLD} 8)${NC} Start production server (npm start)"
        echo -e "  ${BOLD} 9)${NC} Build Docker image"
        echo -e "  ${BOLD}10)${NC} Run with Docker Compose"
        echo -e "  ${BOLD}11)${NC} Lint (npm run lint)"
        echo -e "  ${BOLD}12)${NC} Analyze bundle size (npm run analyze)"
        echo -e "  ${BOLD}13)${NC} View logs"
        echo -e "  ${BOLD}14)${NC} Stop containers"
        echo -e "  ${BOLD}15)${NC} Clean (rm -rf .next node_modules)"
        echo ""
